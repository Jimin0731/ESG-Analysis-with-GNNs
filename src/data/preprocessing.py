...
def _validate_square_matrix(matrix: np.ndarray) -> None:
    if matrix.ndim != 2 or matrix.shape[0] != matrix.shape[1]:
        raise ValueError("I/O matrix must be square with matching source and target industries")
...
def targets_from_esg_frame(esg_df: pd.DataFrame, node_labels: Iterable[str]) -> np.ndarray:
    ...
    if esg_df.empty:
        raise ValueError("ESG input must contain at least one row")
    df = esg_df.copy()
...
def build_graph_dataset(io_matrix: pd.DataFrame | np.ndarray, targets: np.ndarray | None = None, years_used: list[int] | None = None) -> GraphDataset:
    ...
    if targets.shape[0] != features.shape[0]:
        raise ValueError("target length must match the number of graph nodes")

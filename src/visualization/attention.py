...
def plot_attention_edges(explanation,*,top_k=10):
    ...
    weighted=any(r.economic_edge_weight is not None for r in records)
    ax.set_title(f"{explanation.model_name} directed attention ({'weighted' if weighted else 'unweighted'})\nperiod {explanation.period}, layer {explanation.layer_index}, heads {explanation.head_aggregation}")
...
def plot_attention_subgraph(explanation,*,top_k=10,seed=0):
    ...
    ax.set_title("Directed attention subgraph"); ax.axis("off"); fig.text(.5,.01,"Model association/local sensitivity — not a causal estimate",ha="center",fontsize=8); return fig

...
from .contracts import ModelConfig, ModelOutput, ModelValidationError
...
class BidirectionalGNN(nn.Module):
    ...
    def _branch(self,h,edge_index,edge_weight,layers):
        for layer in layers: h=self.drop(self.act(layer(h,edge_index,edge_weight)))
        return h
    def forward(self,x,edge_index=None,edge_weight=None,*,return_aux=False):
        ...
        if edge_index.shape[1] > 0 and edge_weight is None: raise ModelValidationError("edge_weight is required for non-empty bidirectional_gnn graphs")
        if edge_weight is None: edge_weight=torch.empty((0,),device=x.device,dtype=x.dtype)
        f=self._branch(self.act(self.f_in(x)),edge_index,edge_weight,self.forward_layers); r=self._branch(self.act(self.r_in(x)),edge_index.flip(0),edge_weight,self.reverse_layers); c=self.act(self.project(torch.cat([f,r],dim=-1)))
        aux={}
